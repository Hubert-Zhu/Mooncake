...
class MasterServiceHATest : public ::testing::Test {
   protected:
    ...
    // Bytes restored from a standby snapshot before their segment is mounted.
    // They hang off DummyBufferAllocator, invisible to the usage tracker.
    static uint64_t StandbyAccountedBytesForTesting(
        const MasterService& service) {
        uint64_t total = 0;
        for (const auto& entry : service.standby_accounted_memory_bytes_) {
            total += entry.second;
        }
        return total;
    }
...
TEST_F(MasterServiceHATest, RestoreFailureKeepsExistingState) {
    ...
    // Otherwise the comparison below passes on two zeroes.
    ASSERT_GT(accounted_after_restore, 0u);
